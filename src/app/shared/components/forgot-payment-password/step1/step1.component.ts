...
export class forgetStep1Component implements OnInit, OnDestroy {
  constructor(
  ...
  getSmsCode() {
    ...
    this.basicSrv
      .queryMobileSms({ method: SmsMethod.ChangePinCode, mobileArea: mobileArea, mobile: mobile.toString() })
      .subscribe(
        () => {
          const { mobile, mobileArea } = this.validateForm.value;
          this.modal.confirm({
