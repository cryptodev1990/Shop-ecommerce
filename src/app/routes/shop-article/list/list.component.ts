...
export class ListComponent extends DestroySubscription implements OnInit {
  ...
  search() {
    this.loading = true;
    this.articleSrv
      .getArticleList({ rows: this.rows, page: this.page - 1, articleCategoryId: this.articleCategoryId })
      .subscribe({
        next: res => {
          console.log(res);
